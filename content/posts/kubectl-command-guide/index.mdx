...
kubectl get po -o wide                   # Display additional pod info like IP and node
kubectl describe po                      # Show detailed pod information
kubectl get po --show-labels             # List pods with their associated labels
kubectl get po -l app=<app_name>         # Filter pods by specific label, like "app"
kubectl get po -o yaml                   # Output pods in YAML format
```

### Save the YAML of a specific pod

```bash
kubectl get po <pod_name> -o yaml --export > <pod_name>.yaml  # Export pod definition to a file
```
...
```bash
kubectl get svc,po                       // List services and pods
kubectl get deploy,no                    // List deployments and nodes
